...
while(t<614517){
  if(data$Quarter[t]==4 && data$Quarter[t-1]==3 && data$Quarter[t-2]==3){
    ...
    sum2 = sum2 + 1
      ...
      else if(grepl("Defensive rebound", data$AwayPlay[t], fixed=TRUE)){
    ...
    else if(grepl("Defensive rebound", data$HomePlay[t], fixed=TRUE)){
...
for(team in 1:30){
  ...
  PPP[team+0] = (fqy[team]/(fqn[team]+fqy[team]))
  secondQ[team+0] = fourthEasyW[team]/(fourthEasyL[team]+fourthEasyW[team])
}

OReboundPt = PPP
summary(lm(secondQ ~ OReboundPt))
