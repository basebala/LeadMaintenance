...
while(t<614517){
  if(data$Quarter[t]==4 && data$Quarter[t-1]==3 && data$Quarter[t-2]==3){
    ...
    if(grepl("Offensive rebound", data$HomePlay[t], fixed=TRUE)){
      if(grepl("-pt", data$HomePlay[t+1], fixed=TRUE)){
        fqy[curHomeTeam] = fqy[curHomeTeam] + data$SecLeft[t] - data$SecLeft[t+1]
    ...
    if(grepl("Offensive rebound", data$AwayPlay[t], fixed=TRUE)){
      if(grepl("-pt", data$AwayPlay[t+1], fixed=TRUE)){
        fqy[curAwayTeam] = fqy[curAwayTeam] + data$SecLeft[t] - data$SecLeft[t+1]
