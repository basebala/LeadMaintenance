...
while(t<614517){
  if(data$Quarter[t]==4 && data$Quarter[t-1]==3 && data$Quarter[t-2]==3){
    ...
    fqn[curHomeTeam] = fqn[curHomeTeam] + data$HomeScore[t]-data$HomeScore[t-1]
    ...
    fqn[curAwayTeam]= fqn[curAwayTeam] + data$AwayScore[t]-data$AwayScore[t-1]
    if(grepl("o", data$HomePlay[t], fixed=TRUE) && grepl("o", data$AwayPlay[t-1], fixed=TRUE)){
...
for(team in 1:30){
  ...
  PPP[team+90] = fqn[team]/(fqy[team])
  secondQ[team+90] = fourthEasyW[team]/(fourthEasyL[team]+fourthEasyW[team])
}

print(Home3Pt)
OPointsPerPossession = PPP
summary(lm(secondQ ~ OPointsPerPossession))
