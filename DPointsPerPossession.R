...
while(t<614517){
  if(data$Quarter[t]==4 && data$Quarter[t-1]==3 && data$Quarter[t-2]==3){
    ...
    fqn[curHomeTeam] = fqn[curHomeTeam] + data$AwayScore[t]-data$AwayScore[t-1]
    if(grepl("o", data$HomePlay[t], fixed=TRUE) && grepl("o", data$AwayPlay[t-1], fixed=TRUE)){
    }
    else if(grepl("o", data$AwayPlay[t], fixed=TRUE) && grepl("o", data$HomePlay[t-1], fixed=TRUE)){
      fqy[curHomeTeam] = fqy[curHomeTeam] + 1
      
    }
    else if(grepl("Offensive rebound", data$AwayPlay[t], fixed=TRUE)){
      fqy[curHomeTeam] = fqy[curHomeTeam] + 1
    }
    else if(grepl("Offensive rebound", data$HomePlay[t], fixed=TRUE)){
    }
  }
  else if(high==1 && indicator==0 && data$Quarter[t]==4){
    fqn[curAwayTeam]= fqn[curAwayTeam] + data$HomeScore[t]-data$HomeScore[t-1]
    if(grepl("o", data$HomePlay[t], fixed=TRUE) && grepl("o", data$AwayPlay[t-1], fixed=TRUE)){
...
for(team in 1:30){
  ...
  print(fqy[team]/(fqn[team]))
  PPP[team+0] = fqn[team]/(fqy[team])
  secondQ[team+0] = fourthEasyW[team]/(fourthEasyL[team]+fourthEasyW[team])
}

print(Home3Pt)
DPointsPerPossession = PPP
summary(lm(secondQ ~ DPointsPerPossession))
